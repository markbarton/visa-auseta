...
angular.module('angularPayments').factory('Common', [function(){
...
angular.module('angularPayments').factory('Cards', [function(){
    ...
    var cards = [
        ...
        {
            type:'test',
            pattern:/^(42424242424242[0-9][0-9])/,
            format: defaultFormat,
            length: [16],
            cvcLength: [3],
            luhn: false
...
angular.module('angularPayments').factory('_Format',['Cards', function(Cards){
...
angular.module('angularPayments').factory('_Validate', ['Cards', 'Common', function(Cards, Common){
...
//ret = (ref = num.length, __indexOf.call(card.length, ref) >= 0) && (card.luhn === false || _luhnCheck(num));
        ret = (__indexOf.call(card.length, num.length) >= 0) && (card.luhn === false || _luhnCheck(num));
    ...
    .directive('paymentsValidate', ['$window', '_Validate','Cards', function($window, _Validate,Cards){
        return {
            restrict: 'A',
            require: 'ngModel',
            scope:{ctrlFn: '&'},
            link: function(scope, elem, attr, ctrl){
                ...
                elem.bind('keyup', function(e){
                    var isValid=_Validate(type, elem.val());
                    card = Cards.fromNumber(elem.val());

                    if(isValid){
                        scope.ctrlFn({isValid:true,cardNumber:elem.val(),cardObject:card})

                    }else{
                        scope.ctrlFn({isValid:false,cardNumber:elem.val(),cardObject:card})
                    }
                    ctrl.$setValidity(type,isValid );
...
angular.module('angularPayments').directive('stripeForm', ['$window', '$parse', 'Common', function($window, $parse, Common) {
